...
RDS_STAGING_SCHEMA = Variable.get("dwh_staging_schema")
...
dag = DAG(
    dag_id = "sync_staging_schema",
    description = "Sync staging schema with the latest changes",
    default_args = default_args,
    schedule_interval = "0 17 * * *", # at 17:00 UTC every day (which is 00:00 WIB)
    catchup = False
)
...
with dag:
    ...
    sync_schema_task = PythonOperator(
        ...
        op_kwargs={
            "staging_schema": RDS_STAGING_SCHEMA,
            "production_schema": RDS_PRODUCTION_SCHEMA,
            "engine": engine,
            "sync_limit_rows": 200000  # Adjusted limit for staging schema
        },
        provide_context=True
    )
