...
WITH cte AS (
    SELECT *,
        CASE
            ...
            WHEN COUNT(CASE WHEN sex_ploidy_category = 'Mismatch' THEN 1 END) 
                OVER (PARTITION BY id_batch) > 0 THEN 'Fail'
            WHEN COUNT(CASE WHEN sex_ploidy_category = 'No Data' THEN 1 END) 
                OVER (PARTITION BY id_batch) > 0 THEN 'Incomplete Data'
            ELSE 'Pass'
    ...
    FROM (		SELECT
		    ...
		    COALESCE(mgi_sec.id_batch, illumina_sec.id_batch, seq.id_library) AS id_batch,
