...
def fetch_s3_data(**kwargs):
    ...
    json_data = [{'Path': key, 'Name': key.split('/')[-1], 'Size': s3_hook.get_key(key, bucket).size, 'ModTime': s3_hook.get_key(key, bucket).last_modified.isoformat()} for key in json_data if key.endswith(('.cram', '.cram.crai', '.vcf.gz', '.vcf.gz.tbi'))]
    
    # Fetch CSV data
    config_data = s3_hook.read_key('bgsi-data-citus-config', bucket)
    config_old_data = s3_hook.read_key('bgsi-config-pipeline/input/citus', bucket)

    return {
        'json_data': json_data,
        'config_data': config_data,
        'config_old_data': config_old_data
    }
